...
router.post('/apertura', authenticate, requireRole('DESPACHADOR', 'ADMIN'), async (req: Request, res: Response) => {
  const result = await TurnosController.apertura(req.body);
  res.status(result.status).json({ data: result.data, error: result.error });
});

// PATCH /turnos/:id/cierre — DESPACHADOR o ADMIN
router.patch('/:id/cierre', authenticate, requireRole('DESPACHADOR', 'ADMIN'), async (req: Request, res: Response) => {
...
router.patch('/:id/validar', authenticate, requireRole('ADMIN'), async (req: Request, res: Response) => {
