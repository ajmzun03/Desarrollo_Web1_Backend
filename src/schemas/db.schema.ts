...
export const municipioTable = pgTable('MUNICIPIO', { //3 - Angel
...
export const departamentoTable = pgTable('DEPARTAMENTO', { //4 - Angel
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
  departamento: varchar('departamento', { length: 100 }).notNull().unique()
})

export const direccionTable = pgTable('DIRECCION', { //5 - Angel
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const unidadMedidaTable = pgTable('UNIDAD_MEDIDA', { //7 - Angel
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const materiaPrimaTable = pgTable('MATERIA_PRIMA', { //8 - Angel
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const loteMateriaPrimaTable = pgTable('LOTE_MATERIA_PRIMA', { //9 - Angel
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
...
export const kardexBodegaTable = pgTable('KARDEX_BODEGA', { //12 - Angel
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  bodega_id: integer('bodega_id').notNull().references(() => bodegaTable.id),
...
export const stockAlacenaTable = pgTable('STOCK_ALACENA', { //14 - Adrian
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const kardexAlacenaTable = pgTable('KARDEX_ALACENA', { //15 - Adrian
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  alacena_id: integer('alacena_id').references(() => alacenaTable.id),
  lote_id: bigint('lote_id', { mode: 'number' }).references(() => loteMateriaPrimaTable.id),
  tipo_movimiento: movimientoEnum('movimiento').notNull(),
...
export const productoTable = pgTable('PRODUCTO', { //16 - Adrian
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
  categoria_id: integer('categoria_id').notNull().references(() => categoriaTable.id),
  unidad_medida_id: integer('unidad_medida_id').notNull().references(() => unidadMedidaTable.id),
...
export const proveedorTable = pgTable('PROVEEDOR', { //20 - Adrian
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const ordenCompraTable = pgTable('ORDEN_COMPRA', { //21 - Adrian
  id: bigint('id', { mode: "number" }).notNull().primaryKey().generatedByDefaultAsIdentity(),
...
export const hojaRecepcionDetalleTable = pgTable("HOJA_RECEPCION_DETALLE", { //24 - Adrian
  id: bigint('id', { mode: "number" }).primaryKey().generatedByDefaultAsIdentity(),
...
export const facturaCompraTable = pgTable('FACTURA_COMPRA', { //25 Carlos
  id: bigint('id', { mode: 'number' }).notNull().primaryKey().generatedByDefaultAsIdentity(),
...
export const recetaTable = pgTable('RECETA', { //28 Carlos
...
export const hojaDespachoTable = pgTable('HOJA_DESPACHO', { //30 Carlos
  id: bigint('id', { mode: 'number' }).notNull().primaryKey().generatedByDefaultAsIdentity(),
...
export const cajaTable = pgTable('CAJA', { //32 Carlos
  id: integer('id').primaryKey().generatedByDefaultAsIdentity(),
...
export const turnoDespachadorTable = pgTable('TURNO_DESPACHADOR', { //33 Carlos
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
...
export const liquidacionRepartidorTable = pgTable('LIQUIDACION_REPARTIDOR', { //34 Carlos
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
...
export const gastosSucursalTable = pgTable('GASTOS_SUCURSAL', { //35 Carlos
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  caja_id: integer('caja_id').references(() => cajaTable.id), // CAJA.id
  usuario_id: integer('usuario_id').references(() => usuarioTable.id), // USUARIO.id
  sucursal_id: integer('sucursal_id').references(() => sucursalTable.id), // SUCURSAL.id
